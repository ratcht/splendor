...
from actions import TakeTwoGems, TakeThreeGems, BuyCard, ReserveCard, ACTION_CLASSES
...
def test_buy_includes_board_and_reserved():
  ...
  reserved  = card(e=1)
  board = make_board(
    dealt={CardLevel.Level1: [on_board], CardLevel.Level2: [], CardLevel.Level3: []},
  )
...
def test_buy_excludes_unaffordable():
  ...
  expensive = card(r=5)
  board = make_board(
    dealt={CardLevel.Level1: [cheap, expensive], CardLevel.Level2: [], CardLevel.Level3: []},
  )
...
def test_reserve_empty_when_three_reserved():
  ...
  board = make_board(
    dealt={CardLevel.Level1: [some], CardLevel.Level2: [], CardLevel.Level3: []},
  )
...
def test_reserve_no_returns_when_under_10_gems():
  c = card()
  board = make_board(
    dealt={CardLevel.Level1: [c], CardLevel.Level2: [], CardLevel.Level3: []},
...
def test_all_legal_actions_pass_is_valid(cls):
  ...
  c1 = card(level=CardLevel.Level1, gem=Gem.Ruby, points=1, e=1)
  c2 = card(level=CardLevel.Level1, gem=Gem.Onyx, points=0, e=1, s=2)
  c3 = card(level=CardLevel.Level2, gem=Gem.Sapphire, points=2, e=2, r=3)
  board = make_board(
    dealt={CardLevel.Level1: [c1, c2], CardLevel.Level2: [c3], CardLevel.Level3: []},
  ...
  for action in actions:
    assert action.is_valid(board, player), f"{action} from legal_actions failed is_valid"
